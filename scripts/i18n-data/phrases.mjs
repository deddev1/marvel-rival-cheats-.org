...
export const phrases = {
	es: {
		buy: 'Comprar Marvel Rivals Cheats', undetected: 'indetectables', maps: 'Marvel Rivals maps, objectives, and team fight zones',
		...
		s2: () => `Licencias con ${phrases.es.delivery} vía checkout ${KW.checkout}. Planes ${phrases.es.monthly} y ${phrases.es.lifetime} incluyen mantenimiento ${KW.neac}.`,
		s3: () => `Consulta la página Updates antes de jugar tras parches de ${KW.neac}. Ningún cheat garantiza estado indetectable permanente.`,
	...
	fr: {
		...
		s2: () => `Licences avec ${phrases.fr.delivery} via checkout ${KW.checkout}. Forfaits ${phrases.fr.monthly} et ${phrases.fr.lifetime} incluent la maintenance ${KW.neac}.`,
		s3: () => `Consultez Updates après les patchs ${KW.neac}. Aucune triche ne garantit un statut indétectable permanent.`,
	...
	de: {
		...
		s2: () => `Lizenzen mit ${phrases.de.delivery} über ${KW.checkout} Checkout. ${phrases.de.monthly} und ${phrases.de.lifetime} inkl. ${KW.neac}-Wartung.`,
		s3: () => `Updates-Seite nach ${KW.neac}-Patches prüfen. Kein Cheat garantiert dauerhaft undetected Status.`,
	...
	pt: {
		...
		s2: () => `Licenças com ${phrases.pt.delivery} via checkout ${KW.checkout}. Planos ${phrases.pt.monthly} e ${phrases.pt.lifetime} incluem manutenção ${KW.neac}.`,
		s3: () => `Verifique Updates após patches ${KW.neac}. Nenhum cheat garante status indetectável permanente.`,
	...
	it: {
		...
		s2: () => `Licenze con ${phrases.it.delivery} via checkout ${KW.checkout}. Piani ${phrases.it.monthly} e ${phrases.it.lifetime} includono manutenzione ${KW.neac}.`,
		s3: () => `Controlla Updates dopo patch ${KW.neac}. Nessun cheat garantisce stato indetectable permanente.`,
	...
	nl: {
		...
		s2: () => `Licenties met ${phrases.nl.delivery} via ${KW.checkout} checkout. ${phrases.nl.monthly} en ${phrases.nl.lifetime} incl. ${KW.neac}-onderhoud.`,
		s3: () => `Check Updates na ${KW.neac}-patches. Geen cheat garandeert permanent undetected status.`,
	...
	pl: {
		...
		s2: () => `Licencje z ${phrases.pl.delivery} przez checkout ${KW.checkout}. Plany ${phrases.pl.monthly} i ${phrases.pl.lifetime} z konserwacją ${KW.neac}.`,
		s3: () => `Sprawdź Updates po patchach ${KW.neac}. Żaden cheat nie gwarantuje permanentnego undetected statusu.`,
	...
	ru: {
		...
		s2: () => `Лицензии с ${phrases.ru.delivery} через checkout ${KW.checkout}. Тарифы ${phrases.ru.monthly} и ${phrases.ru.lifetime} включают обслуживание ${KW.neac}.`,
		s3: () => `Проверяйте Updates после патчей ${KW.neac}. Ни один чит не гарантирует постоянный undetected статус.`,
	...
	tr: {
		...
		s2: () => `${KW.checkout} checkout ile ${phrases.tr.delivery}. ${phrases.tr.monthly} ve ${phrases.tr.lifetime} planları ${KW.neac} bakımı içerir.`,
		s3: () => `${KW.neac} yamalarından sonra Updates sayfasını kontrol edin. Hiçbir hile kalıcı undetected garantisi vermez.`,
	...
	ar: {
		...
		s2: () => `تراخيص مع ${phrases.ar.delivery} عبر checkout ${KW.checkout}. خطط ${phrases.ar.monthly} و${phrases.ar.lifetime} تشمل صيانة ${KW.neac}.`,
		s3: () => `راجع Updates بعد تص patches ${KW.neac}. لا يضمن أي غش حالة undetected دائمة.`,
	...
	ja: {
		...
		s2: () => `${KW.checkout} checkoutで${phrases.ja.delivery}。${phrases.ja.monthly}と${phrases.ja.lifetime}プランに${KW.neac}メンテナンス含む。`,
		s3: () => `${KW.neac}パッチ後はUpdatesページを確認。永久undetectedは保証されません。`,
	...
	ko: {
		...
		s2: () => `${KW.checkout} checkout으로 ${phrases.ko.delivery}. ${phrases.ko.monthly} 및 ${phrases.ko.lifetime} 요금제에 ${KW.neac} 유지보수 포함.`,
		s3: () => `${KW.neac} 패치 후 Updates 페이지 확인. 영구 undetected 보장 없음.`,
	...
	zh: {
		...
		s2: () => `通过${KW.checkout} checkout${phrases.zh.delivery}。${phrases.zh.monthly}和${phrases.zh.lifetime}含${KW.neac}维护。`,
		s3: () => `${KW.neac}补丁后请查看Updates页面。无任何作弊保证永久undetected。`,
	...
	hi: {
		...
		s2: () => `${KW.checkout} checkout से ${phrases.hi.delivery}। ${phrases.hi.monthly} और ${phrases.hi.lifetime} में ${KW.neac} maintenance शामिल।`,
		s3: () => `${KW.neac} पैच के बाद Updates पेज देखें। कोई भी cheat permanent undetected गारंटी नहीं देता।`,
	...
	id: {
		...
		s2: () => `${phrases.id.delivery} via checkout ${KW.checkout}. Paket ${phrases.id.monthly} dan ${phrases.id.lifetime} termasuk pemeliharaan ${KW.neac}.`,
		s3: () => `Periksa Updates setelah patch ${KW.neac}. Tidak ada cheat yang menjamin undetected permanen.`,
	...
	th: {
		...
		s2: () => `${phrases.th.delivery} ผ่าน checkout ${KW.checkout} แพ็ก ${phrases.th.monthly} และ ${phrases.th.lifetime} รวมการดูแล ${KW.neac}`,
		s3: () => `ตรวจ Updates หลังแพตช์ ${KW.neac} ไม่มี cheat รับประกัน undetected ถาวร`,
	...
	vi: {
		...
		s2: () => `${phrases.vi.delivery} qua checkout ${KW.checkout}. Gói ${phrases.vi.monthly} và ${phrases.vi.lifetime} gồm bảo trì ${KW.neac}.`,
		s3: () => `Kiểm tra Updates sau bản vá ${KW.neac}. Không cheat nào đảm bảo undetected vĩnh viễn.`,
	...
	uk: {
		...
		s2: () => `Ліцензії з ${phrases.uk.delivery} через checkout ${KW.checkout}. Тарифи ${phrases.uk.monthly} і ${phrases.uk.lifetime} включають обслуговування ${KW.neac}.`,
		s3: () => `Перевіряйте Updates після патчів ${KW.neac}. Жоден чит не гарантує постійний undetected статус.`,
	...
	cs: {
		...
		s2: () => `Licence s ${phrases.cs.delivery} přes checkout ${KW.checkout}. Plány ${phrases.cs.monthly} a ${phrases.cs.lifetime} včetně údržby ${KW.neac}.`,
		s3: () => `Po patchích ${KW.neac} zkontrolujte Updates. Žádný cheat negarantuje trvalý undetected stav.`,
	...
	ro: {
		...
		s2: () => `Licențe cu ${phrases.ro.delivery} via checkout ${KW.checkout}. Planuri ${phrases.ro.monthly} și ${phrases.ro.lifetime} includ mentenanță ${KW.neac}.`,
		s3: () => `Verificați Updates după patch-uri ${KW.neac}. Niciun cheat nu garantează undetected permanent.`,
	...
	sv: {
		...
		s2: () => `Licenser med ${phrases.sv.delivery} via ${KW.checkout} checkout. ${phrases.sv.monthly} och ${phrases.sv.lifetime} inkl. ${KW.neac}-underhåll.`,
		s3: () => `Kontrollera Updates marvel-rivalser ${KW.neac}-patchar. Ingen cheat garanterar permanent undetected status.`,
